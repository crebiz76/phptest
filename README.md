...
## 22. 실전 웹사이트 만들기 - 로그인 처리, 회원 정보 수정
- 세션을 이용한 로그인/로그아웃 처리
    - 로그인 후 레벨 표시 안 됨(문제)
- 세션 변수 활용법
- 회원 정보 DB에서 읽어오기
...
## 23. 실전 웹사이듵 만들기 - 낙서장(방명록) 만들기
- 낙서장(방명록) 기능 이해 
- 낙서장 메인글과 댓글 DB 테이블 설계/생성
- 세션 변수 활용하여 특정 사용자에게만 삭제 권한 부여
- 낙서장/댓글의 저장과 삭제 관련 PHP 페이지 작성

- 결과화면과 요구사항
    - 로그인 상태에서만 글쓰기 가능
        - 로그인이 안 되어 있으면, 낙서장의 내용만 보기
        - 로그인이 되어 있으면, 낙서장/댓글의 쓰기 창 보이기
    - 댓글 달기 기능 추가
    - 메인글과 댓글을 한 화면에 출력
    - 닉네임, 작성일 및 시간, 글 내용 출력
    - 작성자와 관리자만 메인글 및 댓글 삭제 가능
...
- 페이지 구성
    - 낙서장 memo.php
    - 메인글 저장 insertMemo.php
    - 메인글 삭제 deleteMemo.php
